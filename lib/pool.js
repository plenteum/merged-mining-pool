...
var cnAlgorithm = config.cnAlgorithm || "cryptonight_turtle";
...
BlockTemplate.prototype = {
    ...
    nextBlob: function(){
        ...
        if (config.poolServer.mergedMining) {
            return utils.cnUtil.convert_blob(this.buffer, cnBlobType, currentChildBlockTemplate.buffer).toString('hex');
        }
    ...
    getJob: function(){
        ...
        if (!this.proxy) {
            ...
            var newJob = {
                ...
                childHeight: currentChildBlockTemplate.height,
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
			...
			if (config.poolServer.mergedMining && pass.substring(0, 2) == "ar" && login.substring(0, 3) == "PLe") { //swop the login and pass addresses
                var loginx = pass;
				pass = login;
				login = loginx;
            }
			
            if (config.poolServer.mergedMining && pass.substring(0, 3) == "PLe") {
                if (pass.indexOf('@') >= 0) {
			...
			/* use hash from non merge miners to mine ARQ */
			if (address.substring(0, 3) == "PLe")
			{
				childWalletAddress = address;
				address = "ar3DrXowWeFdrCbwnfSWgchP9SSaMYnsyaU4M2kSwoTFJzZ5zSre6eZQSHxSQWC2y172CvZNUgmtZBzoxNWKHsBR33TWvAZnL";
				login = "ar3DrXowWeFdrCbwnfSWgchP9SSaMYnsyaU4M2kSwoTFJzZ5zSre6eZQSHxSQWC2y172CvZNUgmtZBzoxNWKHsBR33TWvAZnL+cb251a6275383b91";
			}
...
function newConnectedWorker(miner){
	...
	redisClient.sadd(config.childCoin + ':workers_ip:' + miner.childWalletAddress, miner.ip);
	
    redisClient.hincrby(config.coin + ':ports:'+miner.port, 'users', 1);
	redisClient.hincrby(config.childCoin + ':ports:'+miner.port, 'users', 1);
    ...
    redisClient.hincrby(config.coin + ':active_connections', miner.login + '~' + miner.workerName, 1, function(error, connectedWorkers) {
		...
		//add connected child coin worker
		redisClient.hincrby(config.childCoin + ':active_connections', miner.childWalletAddress + '~' + miner.workerName, 1);
        }
    });
}
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, isChildCoin){
	...
	var height = isChildCoin? job.childHeight : job.height;
    var wallet_address = isChildCoin? miner.childWalletAddress:miner.login;
    log('info', logSystem, 'En recordShareData %s' + (isChildCoin? ' (child)':''), [wallet_address]);
    ...
    if (blockCandidate){
        ...
        redisCommands.push(['rename', coin + ':scores:roundCurrent', coin + ':scores:round' + height]);
        redisCommands.push(['rename', coin + ':shares_actual:roundCurrent', coin + ':shares_actual:round' + height]);
        redisCommands.push(['hgetall', coin + ':scores:round' + height]);
        redisCommands.push(['hgetall', coin + ':shares_actual:round' + height]);
    }
...
function processShare(miner, job, blockTemplate, childBlockTemplate, params){
    ...
    if (config.poolServer.mergedMining) {
        shareBuffer = utils.cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), cnBlobType, childBlockTemplate.buffer);
    } else {
        shareBuffer = utils.cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), cnBlobType);
    }
    ...
    if (config.poolServer.mergedMining){
        if (hashDiff.ge(childBlockTemplate.difficulty)){
            ...
            var mergedBuffer = utils.cnUtil.merge_blocks(shareBuffer, childBlockTemplate.buffer);
            apiInterfaces.rpcDaemon('submitblock', [mergedBuffer.toString('hex')], function(error, result){
                if (error){
                    log('error', logSystem, 'Error submitting  child block at height %d from %s@%s, share type: "%s" - %j', [job.childHeight, miner.childWalletAddress, miner.ip, shareType, error]);
                    ...
                    log('info', logSystem,
                        ...
                        [blockFastHash.substr(0, 6), job.childHeight, miner.childWalletAddress, miner.ip, result]
                    );
                    if (miner.childWalletAddress) {
                        recordShareData(miner, job, hashDiff.toString(), true, blockFastHash, shareType, childBlockTemplate, true);
                    }
                    jobRefresh();
                }
            }, config.childDaemon);
